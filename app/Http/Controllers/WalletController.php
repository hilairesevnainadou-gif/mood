  ...
  /**
    ...
    public function kkiapayCallback(Request $request)
    {
        try {
            ...
            // Données additionnelles (data JSON envoyé par Kkiapay)
            $additionalData = [];
            if (!empty($data['data'])) {
            ...
            // 1. Recherche par transaction_id exact
            if ($transactionId) {
            ...
            // 3. Recherche par données additionnelles
            if (!$transaction && !empty($additionalData)) {
                if (!empty($additionalData['transaction_id'])) {
                    $transaction = Transaction::where('transaction_id', $additionalData['transaction_id'])
                        ->orWhere('reference', $additionalData['transaction_id'])
                        ->first();
                }
                if (!$transaction && !empty($additionalData['reference'])) {
                    $transaction = Transaction::where('reference', $additionalData['reference'])
                        ->orWhere('transaction_id', $additionalData['reference'])
                        ->first();
                }
            }

            // 4. Dernière transaction pending pour ce wallet
            if (!$transaction && !empty($additionalData['wallet_id'])) {
                $transaction = Transaction::where('wallet_id', $additionalData['wallet_id'])
            ...
            // 5. Recherche par montant et téléphone récent
            ...
            if (!$transaction && ($status === 'success' || $status === 'completed')) {
                return $this->createTransactionFromCallback($data, $additionalData);
            }
    ...
    {
        try {
            $walletId = $additionalData['wallet_id'] ?? null;
            $amount = $data['amount'] ?? $additionalData['amount'] ?? 0;
            $phone = $data['phone'] ?? $additionalData['phone'] ?? null;
            ...
            $transaction = Transaction::create([
                'wallet_id' => $wallet->id,
                'transaction_id' => $data['transactionId'] ?? 'KKP-' . strtoupper(Str::random(12)),
                'reference' => $additionalData['reference'] ?? 'DEP-' . date('Ymd') . '-' . strtoupper(Str::random(6)),
                'type' => 'credit',
                'amount' => $amount,
                'total_amount' => $amount,
                'fee' => 0,
     ...
     */
    private function createNotification($userId, array $data)
    {
        try {
            Notification::create([
                'user_id' => $userId,
